...
export class SelectionModel<T>
{
        ...
        values.forEach(value =>
        {
            if (this._compareWith)
            {
                for (const otherValue of this._selection)
                {
                    ...
                    {
                        this._unmarkSelected(otherValue);
                    }
                }
            }
            else
    ...
    /**
     * Determines whether all values are selected.
     */
    isAllSelected(values: T[]): boolean
    {
        for (const value of values)
        {
            let isSelectedCurrentValue = false;
            if (this._compareWith)
            {
                for (const otherValue of this._selection)
                {
            ...
            if (!isSelectedCurrentValue) return false;
        }
        return true;
    }

    /**
     * Determines whether some values are selected.
     */
    isSomeSelected(values: T[]): boolean
    {
        for (const value of values)
        {
            if (this._compareWith)
